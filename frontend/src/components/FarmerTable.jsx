...
function ScoreBadge({ score }) {
  const [bg, text] =
    score >= 70 ? ['bg-green-500/15 border-green-500/30',  'text-green-400'] :
    score >= 50 ? ['bg-yellow-500/15 border-yellow-500/30','text-yellow-400'] :
...
function StatusBadge({ rowStatus, t }) {
  const cfg = {
    approved: { cls: 'bg-green-500/15 border-green-500/30 text-green-400', dot: 'bg-green-400' },
    reserve:  { cls: 'bg-yellow-500/15 border-yellow-500/30 text-yellow-400', dot: 'bg-yellow-400' },
    rejected: { cls: 'bg-red-500/15 border-red-500/30 text-red-400', dot: 'bg-red-400' },
  }[rowStatus] ?? { cls: 'bg-slate-500/15 border-slate-500/30 text-slate-400', dot: 'bg-slate-400' }
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function FarmerTable({ farmers, budget, loading, onSelectFarmer }) {
  const { t } = useLang()
  ...
  const visible = useMemo(() => {
    ...
    return farmersWithStatus
      .filter(f => {
        ...
        if (q && !f.region.toLowerCase().includes(q) &&
                 !f.subsidy_direction.toLowerCase().includes(q) &&
                 !f.app_number.toLowerCase().includes(q)) return false
        return true
      })
      .sort((a, b) => {
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all
                ${filter === f
                  ...
                  : f === 'reserve'  ? 'bg-yellow-600 text-white'
            ...
            {loading
              ...
              : pageSlice.map((f, idx) => (
                <tr
                  ...
                  className={`cursor-pointer transition-colors ${rowBg[f.rowStatus] ?? ''}`}
                >
