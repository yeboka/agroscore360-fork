...
export default function ShapModal({ farmer, onClose }) {
  ...
  const scoreColor =
    farmer.efficiency_score >= 70
      ? "text-green-400"
      : farmer.efficiency_score >= 50
      ? "text-yellow-400"
      : "text-red-400";

  // Shorten long subsidy name for display
  const shortName =
    farmer.subsidy_name?.length > 60
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div
          className="sticky top-0 z-10 bg-surface-card/95 backdrop-blur border-b border-surface-border
        ...
        <div className="flex-1 p-6 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-3">
              <FeatureBar
                label={t("infraIndex")}
                value={farmer.infrastructure_index}
                min={1}
                max={10}
              />
              <FeatureBar
                label={t("survivalRate")}
                value={farmer.herd_survival_rate}
                min={50}
                max={99}
                unit="%"
              />
              <FeatureBar
                label={t("obligations")}
                value={farmer.historical_obligations_met}
                min={0}
                max={1}
              />
              <FeatureBar
                label={t("climateRisk")}
                value={farmer.climate_risk_factor}
                min={0}
                max={1}
                invert
              />
